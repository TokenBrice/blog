...
Cette instance Claude a été conçue en live, donc regardez la vidéo pour le contexte complet. La version courte est qu'elle a reçu comme instruction d'être maximalement adversariale, de ne jamais prendre les déclarations des équipes et des sites officiels pour argent comptant, et de faire confiance à la réalité observable onchain avant tout. Elle porte une attention particulière aux membres d'équipe et à leurs implications passées. Elle a établi elle-même cette hiérarchie de l'information, qui résume bien sa façon de penser :

![information hierarchy](/img/2026/defi-bullshit-detector/information-hierarchy.png)

Nous avons lancé le bot en live sur thGold, avec des instructions spécifiques pour explorer aussi thBILL, un autre produit utilisant la même infrastructure, étant donné la récence de thGold. Plusieurs red flags ont été découverts, notamment [une condamnation pour fraude et détournement de fonds de l'entreprise](https://irblaw.com.sg/learning-centre/mustaq-ahmad-v-ayaz-ahmed-and-others-2024-sghca-17/) du fondateur et principal actionnaire de l'une des cinq contreparties impliquées dans l'infrastructure. [Le rapport complet est accessible ici](https://github.com/TokenBrice/blog/blob/master/static/others/defi-bullshit-detector/theo-thgold.md).

Depuis, j'ai lancé le bot sur deux autres projets, et il a bénéficié de la mise à jour Opus 4.5 ⇒ 4.6, ce qui a produit des résultats encore plus pertinents :

[ETHStrat a été identifié comme un projet à très haut risque avec une confiance moyenne-haute](https://github.com/TokenBrice/blog/blob/master/static/others/defi-bullshit-detector/ethstrat.md). Et pour vérifier que notre bot n'était pas trop sévère avec les projets analysés, je lui ai demandé de mener des recherches sur un projet que je connais bien et que je considère de très grande qualité : [Liquity. Il a conclu à un verdict "Conclusively Positive" avec une confiance élevée](https://github.com/TokenBrice/blog/blob/master/static/others/defi-bullshit-detector/liquity.md).

## Comment le mettre en place

L'enregistrement du live reste la meilleure ressource si vous voulez un walkthrough pas à pas. Je partage simplement ici les fichiers essentiels utilisés. Les blocs ci-dessous sont volontairement conservés en anglais : ce sont des prompts et des règles opérationnelles destinés à être utilisés tels quels par Claude.

### Le `claude.md` racine

{{< details title="Voir le fichier claude.md racine" >}}
...
### Le `claude.md` du dossier `/defi-research/`

Ce fichier a été établi par Claude lui-même, avec nos instructions, pendant le live. Si vous l'importez tel quel, demandez à Claude de le relire et de l'ajuster à votre environnement local, puis vérifiez qu'il dispose bien des outils nécessaires pour interroger les données onchain.

{{< details title="Voir le fichier Claude md DeFi Researcher" >}}
...
J'espère que cet outillage vous sera utile. Vous pouvez désormais disposer d'un chercheur DeFi adversarial de niveau professionnel, pour un effort minimal : tirez-en le maximum\!
